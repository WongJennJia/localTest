...
words = ["house", "bathroom", "bedroom", "living room", "hall", "dining room"]
...
def get_pronunciation_and_ipa(word):
    ...
    if response.status_code == 200:
        ...
        if audio_div:
            ...
            if audio_url:
                # Fix redundant base URL before storing
                if audio_url.startswith("https://www.oxfordlearnersdictionaries.com"):
                    pass  # Keep the URL as it is
                else:
...
# Save to JSON file with plain IPA text (no Unicode escape)
with open("pronunciations.json", "w", encoding='utf-8') as json_file:
    json.dump(pronunciations, json_file, ensure_ascii=False, indent=4)
